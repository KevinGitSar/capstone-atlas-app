                ...
                if(data.length > 0){
                    data.forEach(result =>{
                        if(result.userUsername == username){
                            $("#show-comments").append('<div class="card"><div class="dropdown"><button class="comment-btn" role="button" data-toggle="dropdown" aria-expanded="false">...</button><ul class="dropdown-menu atlas-menu-container"><li><form action="/comment/delete/'+ result.id +'" method="POST"><input type="hidden" name="_token" value="'+ $('meta[name="csrf-token"]').attr('content') +'"/><a href="javascript:;" class="dropdown-item atlas-menu-item m-0" onclick="parentNode.submit()">Delete</a></form></li></ul></div><div class="card-body"><p><strong>'+ result.userUsername +'</strong></p><p>'+ result.comment +'</p></div></div>');
                        } else{
                            $("#show-comments").append('<div class="card"><div class="dropdown"><button class="comment-btn" role="button" data-toggle="dropdown" aria-expanded="false">...</button><ul class="dropdown-menu atlas-menu-container"><li><a href="/report/'+ result.userUsername +'" class="dropdown-item atlas-menu-item m-0">Report</a></li></ul></div><div class="card-body"><p><strong>'+ result.userUsername +'</strong></p><p>'+ result.comment +'</p></div></div>');
                        }
                    });
                }
            }
